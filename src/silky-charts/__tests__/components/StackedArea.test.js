...
test('call onClick', () => {
  ...
  const renderer = create(<StackedArea data={data} onClick={onClick} />, {
    createNodeMock,
  });
  const instance = renderer.root;
  const area = instance.findAllByType(Path)[0];
...
test('call onMouseEnter', () => {
  ...
  const renderer = create(
    <StackedArea data={data} onMouseEnter={onMouseEnter} />,
    {
      createNodeMock,
    }
  );
...
test('call onMouseLeave', () => {
  ...
  const renderer = create(
    <StackedArea data={data} onMouseLeave={onMouseLeave} />,
    {
      createNodeMock,
    }
  );
